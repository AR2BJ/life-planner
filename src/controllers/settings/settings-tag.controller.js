...
export const SettingsTagController = {
  ...
  renderTagsList() {
    ...
    container.innerHTML = tags
      .map((tag) => {
        const usageCount = plans.filter(
          (plan) => Array.isArray(plan.tags) && plan.tags.includes(tag.id),
        ).length;
          ...
          >
            ...
            <div class="w-full xs:w-fit flex items-center gap-1 shrink-0">
              <div
                ...
                title="Used in ${usageCount} plan${usageCount === 1 ? "" : "s"}"
              >
                <i class="fa-regular fa-list-check text-[9px] sm:text-xs text-brand/80"></i>
                <span>${usageCount} ${usageCount <= 1 ? "plan" : "plans"}</span>
              </div>
              <button
  ...
  handleAddTag() {
    ...
    StateManager.save(StateManager.getPlans(), updatedTags);
  ...
  handleSaveTagEdit(tagId, newNameRaw, btn, nameInput) {
    ...
    if (tag) {
      tag.name = newName;
      StateManager.save(StateManager.getPlans(), currentTags);
  ...
  openTagDeleteModal(tagId) {
    ...
    const usageCount = currentPlans.filter(
      (plan) => Array.isArray(plan.tags) && plan.tags.includes(tagId),
    ).length;
    ...
    if (msgEl) {
      if (usageCount > 0) {
        msgEl.innerHTML = `Are you sure you want to delete <strong class="text-color">"<i class="fa-regular fa-tag text-sm me-1"></i>${targetTag.name}"</strong>? <br/><br/> It is currently used in <span class="text-red-500 font-semibold">${usageCount} plan(s)</span>.`;
      } else {
  ...
  executeDeleteTag() {
    ...
    const updatedPlans = currentPlans.map((plan) => {
      if (Array.isArray(plan.tags) && plan.tags.includes(tagId)) {
        return {
          ...plan,
          tags: plan.tags.filter((id) => id !== tagId),
        };
      }
      return plan;
    });

    StateManager.save(updatedPlans, updatedTags);
    this.closeTagDeleteModal();
    ...
    NotificationService.show({
      ...
      undoAction: () => {
        ...
        StateManager.save(currentPlans, restoredTags);
        this.renderTagsList();
        PlanController.refreshUI();
