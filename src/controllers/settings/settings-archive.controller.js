...
export const SettingsArchiveController = {
  ...
  runAutoArchivePipeline() {
    ...
    plans.forEach((plan) => {
      if (plan.archived === true) return;

      const allActivityDates = [...(plan.completedDates || [])];
      let lastActivityDateStr = plan.createdAt;
    ...
    if (modified) {
      ...
      NotificationService.show({
        type: "info",
        message:
          "Stale plans exceeding 30 days structural limits auto-archived",
        icon: "fa-box-archive",
        iconColor: "text-brand/80",
        duration: 5000,
      });
